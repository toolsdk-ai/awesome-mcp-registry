		...
		try {
			...
			if (mcpServerConfig.runtime === "python") {
				...
				if (!isValidPEP508(packageName)) {
					console.log(
						`✗ Skipping invalid package name: ${packageName} (${value.path})`,
					);
					continue;
				}
				...
				if (!(await isPackageOnPyPI(packageName))) {
					console.log(
						`✗ Skipping package not found on PyPI: ${packageName} (${value.path})`,
					);
					continue;
				}
				...
				console.log(
					`✓ Added package ${pythonPackageCount}: ${fullName} (${value.path})`,
				);
			}
		} catch (error) {
...
async function main() {
	...
	console.log(
		`Collected ${uniquePythonPackages.length} unique Python packages`,
	);
